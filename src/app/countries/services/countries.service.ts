...
export class CountriesService {
    ...
    private apiUrl: string= 'https://restcountries.com/v3.1'
     ...
     public cacheStore:CacheStore = {
        byCapital  :{termi  :'',countries:[]},
        byCountries:{termi:'',countries:[]},
    ...
    searchCapital(termi: string): Observable<Country[]> {
        const url= `${this.apiUrl}/capital/${termi}`;
           ...
           .pipe(
           tap( countries => this.cacheStore.byCapital= { termi, countries}),
    ...
    searchCountry( termi : string): Observable<Country[]>{
     ...
     return this.getCountriesRequest(url)
     .pipe(
        tap( countries => this.cacheStore.byCountries= { termi, countries}),
        tap( () => this.saveToLocalStorage())
        );
    }
